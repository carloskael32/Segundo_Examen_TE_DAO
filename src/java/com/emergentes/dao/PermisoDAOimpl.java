...
public class PermisoDAOimpl extends ConexionDB implements PermisoDAO{
    ...
    public Permiso getById(int id) throws Exception {
        ...
        try {
            this.conectar();
            PreparedStatement ps=this.conn.prepareStatement("SELECT p.*, u.usuario as 'usuario', r.descripcion as 'rol' FROM usuarios u, roles r, permisos p where p.id_usuario=u.id AND p.id_rol=r.id and p.id=?;");
            ps.setInt(1, id);
